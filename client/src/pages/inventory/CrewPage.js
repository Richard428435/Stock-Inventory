...
export default function CrewPage() {
  ...
  useEffect(() => {
    try {
      ...
      toast.error('Could not save image (too large). Try a smaller file.');
    }
  }, [teamState]);
  ...
  const handleFileChange = (e) => {
    ...
    if (file && activeUploadId) {
      if (file.size > 4 * 1024 * 1024) {
        return toast.error('File too large. Please use an image under 4MB.');
      }
      ...
      reader.onloadend = async () => {
        try {
          ...
          setTeamState(prev => prev.map(m => m.id === activeUploadId ? { ...m, avatar: res.data.url } : m));
