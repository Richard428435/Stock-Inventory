...
const ItemCard = React.memo(({ item, onEdit, onDelete, onAdjust, onPrint, onView, hasPermission }) => {
...
const ItemListRow = React.memo(({ item, onEdit, onDelete, onAdjust, onPrint, onView, hasPermission }) => {
  const [localShowDelete, setLocalShowDelete] = useState(false);
  const isLowStock = item.quantity <= item.lowStockThreshold;
    ...
    <div className="bg-[#0a0a0a]/80 backdrop-blur-md group flex flex-col md:flex-row items-start md:items-center justify-between p-6 md:p-8 rounded-[2rem] border border-white/5 transition-all duration-500 mb-4 shadow-[0_10px_30px_-10px_rgba(0,0,0,0.8)] relative cursor-pointer hover-lux hover:border-[#c49a5b]/40 hover:shadow-[0_15px_40px_-10px_rgba(196,154,91,0.15)]" onClick={onView}>
      ...
      <div className="flex items-center gap-8 w-full md:w-auto">
        ...
        <div className="flex-1">
          ...
          <div className="flex items-center gap-4 text-[9px] font-bold uppercase tracking-[0.2em] text-white/40">
  ...
  return (
    <>
      ...
      <div className="space-y-6">
        ...
        <section className="flex items-center gap-3 overflow-x-auto inventory-scrollbar pb-4 pt-1">
          <button
            onClick={() => setCategory('')}
            className={`whitespace-nowrap px-8 py-3.5 rounded-full font-bold text-[9px] uppercase tracking-[0.2em] transition-all duration-300 ${
              category === '' || category === 'Media & Broadcasting'
                ? 'bg-gradient-to-r from-[#d1a66a] to-[#b78645] text-[#1a1a1a] shadow-[0_5px_15px_rgba(196,154,91,0.3)] scale-105'
                : 'bg-[#0a0a0a]/80 backdrop-blur-md border border-white/5 text-white/50 hover:bg-white/5 hover:text-white'
            }`}
          >
            All Inventory M&B
          </button>
          {categories.filter(c => c.name !== 'Media & Broadcasting').map(c => (
            <button
              ...
              className={`whitespace-nowrap px-8 py-3.5 rounded-full font-bold text-[9px] uppercase tracking-[0.2em] transition-all duration-300 ${
                category === c.name
                  ? 'bg-gradient-to-r from-[#d1a66a] to-[#b78645] text-[#1a1a1a] shadow-[0_5px_15px_rgba(196,154,91,0.3)] scale-105 border-transparent'
                  : 'bg-[#0a0a0a]/80 backdrop-blur-md border border-white/5 text-white/50 hover:bg-white/5 hover:text-white'
              }`}
            >
              {c.name}
