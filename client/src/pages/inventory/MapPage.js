...
export default function MapPage() {
  ...
  const getItemsInZone = (zoneId) => {
    const loc = (i) => (i.location || '').toLowerCase();
    
    if (zoneId === 'media_cabin') {
      // If Global and Media Cabin were identical, we show all items that don't explicitly belong to the other zones.
      return items.filter(i => {
        const l = loc(i);
        if (l.includes('underground') || l.includes('storage')) return false;
        if (l.includes('kids') || l.includes('koinonia')) return false;
        if (l.includes('stage')) return false;
        return true;
      });
    }
    if (zoneId === 'stage') return items.filter(i => loc(i).includes('stage'));
    if (zoneId === 'underground') return items.filter(i => loc(i).includes('underground') || loc(i).includes('storage') || loc(i).includes('cabin/under'));
    if (zoneId === 'kids') return items.filter(i => loc(i).includes('kids') || loc(i).includes('koinonia'));
    
    return items.filter(i => !i.location);
  };
                 ...
                 activeItems.slice(0, 15).map((item, idx) => (
                   <div key={item._id} className="flex items-center gap-4 p-4 rounded-2xl bg-white/5 border border-transparent hover:border-white/10 transition-colors animate-slide-up" style={{ animationDelay: `${idx * 50}ms` }}>
                      ...
                      <div className="flex-1 min-w-0">
                         ...
                         <p className="text-[9px] text-white/40 uppercase tracking-widest truncate">{item.sku} • {item.quantity} Units</p>
                      </div>
